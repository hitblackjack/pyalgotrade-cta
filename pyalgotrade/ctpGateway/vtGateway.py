...
class VtGateway(object):
    ...
    def __init__(self, gatewayName):
        ...
        self.__TickEvent = observer.Event()

        self.__TradeEvent = observer.Event()
        self.__OrderEvent = observer.Event()
        self.__PositionEvent = observer.Event()
        self.__AccountEvent = observer.Event()
        self.__ContractEvent = observer.Event()

        self.__LogEvent = observer.Event()
        self.__ErrorEvent = observer.Event()

    def getTickEvent(self):
        return self.__TickEvent
    ...
    def getPositionEvent(self):
        return self.__PositionEvent

    def getAccountEvent(self):
        return self.__AccountEvent

    def getContractEvent(self):
        return self.__ContractEvent

    def getLogEvent(self):
        return self.__LogEvent

    def getErrorEvent(self):
        return self.__ErrorEvent

        
    #----------------------------------------------------------------------
    def onTick(self, tick):
        """市场行情推送"""
        self.onLog('there is a tick')
        self.__DataEvent.emit(tick)
    
    #----------------------------------------------------------------------
    def onTrade(self, trade):
        """成交信息推送"""
        self.__TradeEvent.emit(trade) 
    #----------------------------------------------------------------------
    def onOrder(self, order):
        """订单变化推送"""
        self.__OrderEvent.emit(order) 
    #----------------------------------------------------------------------
    def onPosition(self, position):
        """持仓信息推送"""
        self.__PositionEvent.emit(order)
    
    #----------------------------------------------------------------------
    def onAccount(self, account):
        """账户信息推送"""
        self.__AccountEvent.emit(account)
    
    #----------------------------------------------------------------------
    def onError(self, error):
        """错误信息推送"""
        self.__ErrorEvent.emit(error)
        
    #----------------------------------------------------------------------
    def onLog(self, log):
        """日志推送"""
        # 通用事件
        self.__LogEvent.emit(log)
        
    #----------------------------------------------------------------------
    def onContract(self, contract):
        """合约基础信息推送"""
        self.__ContractEvent.emit(contract) 
    #----------------------------------------------------------------------
    def connect(self):
        """连接"""
        pass
